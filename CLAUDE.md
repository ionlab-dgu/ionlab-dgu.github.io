...
- **한 세션에서 여러 커밋을 연속으로 진행하는 경우에도 예외 없이 브랜치 → PR입니다.**
  각 커밋이 서로 독립적인 기능을 다뤄서 "이 정도는 그냥 main에 쌓아도 되겠다"고
  판단하기 쉬운데, 독립적인지 여부와 브랜치·PR 필요 여부는 별개입니다. 커밋을
  몇 개로 나눌지와 어디에 올릴지는 다른 질문입니다 — 커밋은 여러 개로 나누되,
  전부 브랜치 하나에 쌓고 마지막에 PR 하나로 올리세요.
  > ⚠️ **주의 사례 (2026-09-10)**: 세션 하나에서 서로 독립적인 기능 5개(Research
  > Directions 신설, Publication 스키마 확장, Conference 확장, Workshop 트래킹,
  > Slack 로깅)를 커밋 5개로 나눠 진행하면서, 각 커밋이 독립적이라는 이유로 전부
  > `main`에 직접 커밋했습니다. `src/`를 건드리는 커밋이 셋 포함돼 있었는데도
  > PR 리뷰를 거치지 않았습니다. 이후로는 세션을 시작할 때 먼저 브랜치부터 만드세요.
