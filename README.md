...
`main`에 푸시하면 GitHub Actions가 `pnpm build:public`으로 빌드해 GitHub Pages에 올립니다.
**공개 배포에는 private 콘텐츠가 절대 포함되지 않습니다.**

> **전제**: `astro.config.mjs`의 `site`가 `https://ionlab-dgu.github.io`(루트, base 없음)로
> 설정돼 있습니다. 이 저장소가 org 사이트 저장소(`ionlab-dgu.github.io`)가 아니라면
> 배포 대상 저장소를 지정하거나 `site`/`base`를 바꿔야 합니다.
