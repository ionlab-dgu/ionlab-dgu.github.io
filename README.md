...
- [IMPLEMENTATION.md](./IMPLEMENTATION.md) — **구현 상세·스키마·함정.**
  실제 구현된 스키마·라우팅과 **개발하며 겪은 함정**이 적혀 있습니다.
  코드를 고치기 전에 훑어보세요.
- [CLAUDE.md](./CLAUDE.md) — 콘텐츠 스키마, 명명 규칙, private 격리 원칙, 커밋·PR 컨벤션
- [핸드북](./content/handbook/) — 랩에서 일하는 방식 (온보딩·정책·튜토리얼)

> 두 문서의 차이: `labos-handoff.md`가 **지금 무엇을 하기로 했는가**(결정·현황·로드맵),
> `IMPLEMENTATION.md`가 **코드가 왜 이렇게 되어 있는가**(스키마·라우팅·함정)를 다룹니다.
