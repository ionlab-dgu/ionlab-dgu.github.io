...
```bash
git clone git@github.com:ionlab-dgu/lab-os.git
cd lab-os
pnpm install
pnpm dev
```
...
```
lab-os (소스)  ──push to main──▶  GitHub Actions
                                     │ pnpm build:public   (PUBLIC_ONLY=1)
                                     │ 격리 검사
                                     ▼
                            ionlab-dgu.github.io (배포 결과물)
                                     │
                                     ▼
                            https://ionlab-dgu.github.io
```

`main`에 푸시하면 `.github/workflows/deploy.yml`이 공개 빌드를 만들어
별도의 `ionlab-dgu.github.io` 저장소로 push합니다.
**공개 배포에는 private 콘텐츠가 포함되지 않습니다** — 워크플로가 private 저장소를
checkout 하지 않고, `PUBLIC_ONLY=1`로 빌드하며, 배포 직전에 격리 검사를 통과해야 합니다.

### 최초 설정 (한 번만)

1. `ionlab-dgu/ionlab-dgu.github.io` 저장소를 만듭니다 (없다면).
2. 그 저장소에 push할 수 있는 **PAT**를 발급합니다.
   GitHub → Settings → Developer settings → Personal access tokens →
   Fine-grained tokens → 대상 저장소 선택 → **Contents: Read and write**
3. 이 저장소의 Settings → Secrets and variables → Actions 에
   **`PAGES_DEPLOY_TOKEN`** 이름으로 등록합니다.
4. `ionlab-dgu.github.io` 저장소의 Settings → Pages 에서 소스를 `main` 브랜치로 지정합니다.

공개 캘린더를 사이트에 노출하려면 `GCAL_ICAL_SEMINARS` 시크릿도 등록하세요.
